...
cg = [4.4824 0 0];  %Aircraft center of gravity
% Aerodynamic center vertical position of the components
y_ac_wing = 0.5;
y_ac_empennage = 0;
y_ac_fin = 0.7;
y_ac_thrust = 0;

%Position of the center of gravity of all the components SHOULD BE
%AERODYNAMIC CENTER
...
%The back of the wing is called section A 

%Aircraft:
Aircraft.W = sum(W)*9.18;       %Aircraft weight [N]
Aircraft.I_theta = ;            %Aircraft inertia [kg.m²]
Aircraft.C_DB = ;               %Aicraft body drag [N]
Aircraft.l_DB = ;               %Aicraft drag lever arm [m]
...
Wing.l = sqrt((y_ac_wing - cg(2))^2 + (xarm(2) - cg(1))^2); %Wing lever arm [m]
Wing.aoa = atan2(y_ac_wing - cg(2), xarm(2) - cg(1));       %Angle of attack between the wing lever arm and the chord [rad]
...
Empennage.l = sqrt((y_ac_empennage - cg(2))^2 + (xarm(3) - cg(1))^2);   %Empennage lever arm [m]
Empennage.T = 8210;                                                     %Thrust placed on Empennage [N] TO BE CHECKED
Empennage.l_T = sqrt((y_ac_thrust - cg(2))^2 + (xarm(4) - cg(1))^2);    %Empennage thrust lever arm [m]
Empennage.aoa = atan2(y_ac_empennage - cg(2), xarm(3) - cg(1));         %Angle of attack between the empennage lever arm and the chord [rad]
Empennage.aoa_T = atan2(y_ac_thrust - cg(2), xarm(4) - cg(1));          %Angle of attack between the thrust lift lever arm and the chord [rad]
...
Fin.l = sqrt((y_ac_fin - cg(2))^2 + (xarm(3) - cg(1))^2);   %Fin lever arm [m]
...
Tail.W = W(3)*9.81;             %Mass of the v-tail [N]
