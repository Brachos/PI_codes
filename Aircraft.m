function [MTOW, cgT, prop_lift, WING, V_TAIL, RUDDER, FUSELAGE, WEIGHT, PARAM,COST, LongDeriv, LatDeriv, Long_modes, LatModes] = Aircraft(MTOW, cg, prop_lift, drag)
...
MTOW = sum(W);
PayW = sum(W)-W(8);
fprintf('Weight without fuel is %.4f\n',PayW);
% PayW = sum(W)-W(8);
FW = sum(W)-W(7);
fprintf('Weight without payload is %.4f\n',FW);
W_empty = sum(W) - W(7) - W(8);
fprintf('Empty weight is %.4f\n',W_empty);
...
xcg_l2= 4.89; %for the second landing gears
...
x_w = 3.66; %position of the wings
...
beta = sqrt(1-M^2);
kT = cl_alphaT/(2*pi);
CL_alphaT = (2*pi*AR_T)/(2 + sqrt(AR_T^2*beta^2/kT^2*(1 + tan(Lambda_T)^2/beta^2) + 4));
a1 = 4; %CL_alpha tail
r = lwt/(bw/2); 
m = zwt/(bw/2);
de_dAOA = 0.45; %Variation de l'angle epsilon en fonction de l'angle d'attaque
de_dAOA1 = 1.75*a/(pi*7*(2*lwt*0.3/bw)^(1/4)*(1+m));
fprintf('de_dAOA1 is %.4f\n',de_dAOA1);
mfus = (x_w+1/4*cw_root)/l_f; % Parameter to compute the fuselage contribution ~ 0.46
fprintf('Parameter for the fuselage contribution %.4f\n', mfus);
kfus = 0.487+(0.688-0.487)*0.6; % kfus ~ 0.61
fprintf('kfus for the fuselage contribution %.4f\n', kfus);
dCmfus_dCLw = kfus*D_f_max^2*l_f/(Sw*cw_MAC*CLw_alpha); % Fuselage contribution
fprintf('Fuselage contribution for the neutral point is %.4f\n',dCmfus_dCLw);
hn = h0 + V_hT*a1/a*(1-(de_dAOA1))-dCmfus_dCLw; %position of the neutral point in %MAC
fprintf('Neutral point in percent of MAC : %.2dm\n', hn);
...
vAC = xac_v + x_vLE;
X_w = h*cw_MAC-xw_AC;
fprintf('Aerodynamic center of the plane %.4f\n',0.27*cw_MAC+x_wLE);
%% Pitching moment equation
static_stability = h-(h0+V_hT*a1/a*(1-(de_dAOA1))-dCmfus_dCLw); %in the case of the MTOW dCm/dalpha
...
[LongDeriv,Long_modes] = long_dyn_stab(MTOW,...
    ...
    de_dAOA1,static_stability,AOA,alpha_L0,l_f,V_f,l_cg,sweep,Lambda_T*pi/180,...
    cl_alphaw,l_arm,V_hT,cw_MAC,x_wLE,X_w,cw_root,xw_AC,xac_h,cgT(1),Inertia, drag, prop_lift);
