% function [b,S,CL,CD,D,c_root,c_tip,c_AC,x_AC,y_AC,V_fuel] = wing(Mach,Altitude,Mass,AR)
...
AOA  = -0.5*pi/180; % AOA where the drag is minimum or cl/cd is maximum 
cd   = 0.007;       % Minimum drag of this airfoil at Re = 1e6
                    % !!! We have Re ~ 1e7
cl   = 0.53;        % Airfoil lift coefficient with max(cl/cd) = 81.2 
...
% CL and CD
cl_alpha  = (0.9-0.3)*180/(pi*(1.5+2));
alpha_l0  = -(0.8/cl_alpha - 1.5*pi/180);
alpha_01  = -0.23;     % See L5 - P30
theta_tip = 0;%-2*pi/180; % Twist angle
