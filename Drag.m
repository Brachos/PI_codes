...
et = abs(WING.theta_tip *180/pi); %Twist angle [deg](doit être positif)
tc = 0.12; %Thickness to chord ratio (depend du profilé d'airfoil choisi)
...
Vf = 6.54; % Volume of the fuselage ->première approx voir CAD 
Ac = FUSELAGE.a_el * FUSELAGE.b_el * pi; %Cross section area 
lf = FUSELAGE.l_f; %fuselage length 
Sfwet = 25.26; %fuselage wetted area -> CAD
Snwet = 5.68/2; %nacelle wetted area -> CAD
Spwet = 0.8*pi*1.58; %engine wetted area (disq of engine) ->>>>>>>>>>> ENGINE CARACT
...
e = 0.8; % Coef d'oswald ?????? 
CLi = 0.4; % J'ai pas bien compris (mis à 0.36 dnas le code de léo)
clmax = 1.5; %PARAM.CL; % DEMANDER LEO 
CLmax = 1.5; %PARAM.CL; % DEMANDER LEO 
...
%From graph 
delta = 0.0015;% Voir figure 1 page 514 du pdf
Col = ((1.47*10^(-5) + 1.15*10^(-5))/(2*beta));%Voir figure 2 page 515 du pdf
...
A1 = 8; %Figure 13 page 524 du pdf ??????????????????????
A2 = 0; %Figure 13 page 524 du pdf ???? QUID PAVEL ?
Cci = 4.5 *cr; %Page 510 du livre ???????????????????????
...
%phif = 2.2/(sigmaeff^1.5) + 3.8/(sigmaeff^3) Not valid here
phif = 0.25; %better approx
